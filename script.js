...
function applyPromo() {
    ...
    if (input === "ostad10") {
        ...
        promoApplied = true;
    } else if (input === "ostad50") {
        promoDiscount = 0.50;
        message.textContent = "50% discount applied!";
        message.style.color = "green";
        promoApplied = true;
    } else {
    ...
    reloadCart(); // Recalculate totals with discount
}
...
function reloadCart() {
    ...
    let discountedTotal = totalPrice + (totalPrice * 0.12);
    if (promoDiscount > 0) {
        discountedTotal = discountedTotal * (1 - promoDiscount);
    }
    total.innerText = discountedTotal.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2});
}
